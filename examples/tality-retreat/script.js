...
(function () {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    // Slide-clicker keys always work, even with focus in a textarea — clickers
    // typically send PageUp/PageDown and these keys aren't typed by users.
    if (e.key === 'PageDown') { e.preventDefault(); next(); return; }
    if (e.key === 'PageUp')   { e.preventDefault(); prev(); return; }

    // Other shortcuts are blocked when typing in a field
  ...
  // ---------- Capture boards (localStorage + export) ----------
  // Each capture board persists to its own localStorage key (data-capture-key)
  // and exports to its own filename (data-capture-filename). Buttons are scoped
  // to the closest .capture-slide ancestor.
  document.querySelectorAll('[data-capture-board]').forEach(captureBoard => {
    const key = captureBoard.dataset.captureKey || 'nbc-workshop-capture-board';
    const fname = captureBoard.dataset.captureFilename || 'capture';
    const scope = captureBoard.closest('.capture-slide') || captureBoard.parentElement;

    const stored = localStorage.getItem(key);
    if (stored !== null) captureBoard.value = stored;
    captureBoard.addEventListener('input', () => {
      localStorage.setItem(key, captureBoard.value);
    });

    const exportBtn = scope && scope.querySelector('[data-capture-export]');
    if (exportBtn) {
    ...
    const clearBtn = scope && scope.querySelector('[data-capture-clear]');
    if (clearBtn) {
