...
- Need a logo? `logos/`. Dark variants for ink surfaces (the default). Light variants for off-white documents only.
...
For implementation:
- The brand defaults to ink black (`#0E0E0F`) on every digital surface - decks, web, social. Don't introduce off-white into digital surfaces.
...
**Default surface:** Ink black (`#0E0E0F`). The brand defaults dark on every digital surface.
...
2. Oversized "racing number" section markers in muted red
...
Tag releases when the brand makes a significant move (new typography, surface direction, new mark). Current: v1.0.0 - the dark/graphic direction with monospace labels.
